...
// node_modules/openpipe/node_modules/form-data/lib/browser.js
var require_browser = __commonJS({
  "node_modules/openpipe/node_modules/form-data/lib/browser.js"(exports, module) {
...
var maxInt, base, tMin, tMax, skew, damp, initialBias, initialN, delimiter, regexPunycode, regexNonASCII, regexSeparators, errors, baseMinusTMin, floor, stringFromCharCode, ucs2encode, basicToDigit, digitToBasic, adapt, decode, encode2, toUnicode, toASCII, punycode, punycode_es6_default;
...
var EventStream = class {
  ...
  _emit(event, ...args) {
    ...
    if (event === "abort") {
      ...
      __classPrivateFieldGet3(this, _EventStream_rejectConnectedPromise, "f").call(this, error2);
      __classPrivateFieldGet3(this, _EventStream_rejectEndPromise, "f").call(this, error2);
      this._emit("end");
      return;
    }
    if (event === "error") {
...
// node_modules/openpipe/openai.js
...
function mergeChunks(base2, chunk) {
  var _a2, _b, _c, _d, _e;
  if (base2 === null) {
    return mergeChunks(
        ...
        choices: chunk.choices.map((c) => {
          ...
          return {
            ...
            delta: {
              ...
              tool_calls: (_a3 = c.delta.tool_calls) == null ? void 0 : _a3.map((tc) => ({
                ...tc,
                function: {
                  ...tc.function
                }
  ...
  for (const choice of chunk.choices) {
    ...
    if (baseChoice) {
      ...
      if ((_b = choice.delta) == null ? void 0 : _b.function_call) {
      ...
      if ((_c = choice.delta) == null ? void 0 : _c.tool_calls) {
        const toolCalls = baseChoice.message.tool_calls ?? [];
        const toolCallDelta = { ...choice.delta.tool_calls[0] };
        if ((_d = toolCallDelta == null ? void 0 : toolCallDelta.function) == null ? void 0 : _d.name) {
          toolCalls.push({
            id: toolCallDelta.id,
            type: "function",
            function: {
          ...
          toolCalls[toolCalls.length - 1].function.arguments += ((_e = toolCallDelta.function) == null ? void 0 : _e.arguments) ?? "";
        }
...
var getFormData = (options) => {
  if (options.formData) {
    ...
    const process2 = (key, value) => {
      if (isString(value) || isBlob2(value)) {
...
var getHeaders2 = async (config, options) => {
  ...
  return new Headers3(headers);
};
var getRequestBody = (options) => {
  var _a2;
  if (options.body !== void 0) {
    if ((_a2 = options.mediaType) == null ? void 0 : _a2.includes("/json")) {
      return JSON.stringify(options.body);
    } else if (isString(options.body) || isBlob2(options.body) || isFormData(options.body)) {
      return options.body;
    } else {
      return JSON.stringify(options.body);
    }
...
var sendRequest = async (options, url, body, formData, headers, onCancel) => {
  ...
  return await lib_default(url, request2);
};
...
var catchErrorCodes = (options, result) => {
  ...
  const error2 = errors2[result.status];
  if (error2) {
...
var OPClient = class {
  constructor(config, HttpRequest = NodeHttpRequest) {
    this.request = new HttpRequest({
      BASE: (config == null ? void 0 : config.BASE) ?? "https://api.openpipe.ai/api/v1",
      VERSION: (config == null ? void 0 : config.VERSION) ?? "0.1.1",
...
var DefaultSdkTags = ((DefaultSdkTags2) => {
...
var OpenPipe = class {
  constructor(config = {}) {
    const openPipeApiKey = (config == null ? void 0 : config.apiKey) ?? readEnv("OPENPIPE_API_KEY");
    const openpipeBaseUrl = (config == null ? void 0 : config.baseUrl) ?? readEnv("OPENPIPE_BASE_URL");
...
var WrappedBeta = class extends Beta {
  constructor(client) {
    super(client);
    this.chat = new WrappedBetaChat(client);
  }
};
var WrappedBetaChat = class extends Chat2 {
  constructor(client) {
    super(client);
    this.completions = new WrappedBetaCompletions(client);
  }
};
var WrappedBetaCompletions = class extends Completions2 {
  constructor(client) {
    super(client);
  }
  parse(body, options) {
...
var OpenAI2 = class extends OpenAI {
  constructor({ openpipe, ...options } = {}) {
    ...
    const fallbackClient = openpipe && "fallbackClient" in openpipe && openpipe.fallbackClient instanceof OpenAI ? openpipe.fallbackClient : new OpenAI({ ...options, apiKey: this.apiKey });
...
var WrappedChat = class extends OpenAI.Chat {
  constructor() {
    super(...arguments);
...
var WrappedCompletions = class extends OpenAI.Chat.Completions {
  constructor(client) {
    super(client);
    ...
    if (usedBody.stream) {
      try {
        return new WrappedStream(response, (response2) => {
          var _a3;
          if (!((_a3 = openpipeArgs.openpipe) == null ? void 0 : _a3.logRequest)) return Promise.resolve();
          return this._report({
            requestedAt,
            receivedAt: Date.now(),
            reqPayload: usedBody,
      ...
      reportingFinished = ((_a2 = openpipeArgs.openpipe) == null ? void 0 : _a2.logRequest) ? this._report({
        requestedAt,
        receivedAt: Date.now(),
        reqPayload: usedBody,
  ...
  async _handleResponseError(error2, usedBody, openpipeArgs, requestedAt) {
    let reportingFinished = Promise.resolve();
    if (error2 instanceof APIError || error2 instanceof ApiError) {
      const rawMessage = error2 instanceof APIError ? error2.message : error2.body.message;
      const message = Array.isArray(rawMessage) ? rawMessage.join(", ") : rawMessage;
      reportingFinished = this._report({
        requestedAt,
        receivedAt: Date.now(),
        reqPayload: usedBody,
        respPayload: error2 instanceof APIError ? error2.error : error2.body,
  ...
  async create({ openpipe: rawOpenpipe, ...body }, options) {
    ...
    if (body.model.startsWith("openpipe:") || body.model.startsWith("openai:") || body.model.startsWith("anthropic:") || body.model.startsWith("gemini:") || (openpipe == null ? void 0 : openpipe.cache)) {
      ...
      try {
        const response = await this.opCompletionClient.chat.completions.create(body, {
          ...
          headers: {
            ...
            "op-cache": openpipe == null ? void 0 : openpipe.cache,
        ...
        if ((_a2 = openpipe.fallback) == null ? void 0 : _a2.model) {
          ...
          try {
            const response = await this.fallbackClient.chat.completions.create(fallbackBody, {
              ...options,
              timeout: openpipe.fallback.timeout ?? (options == null ? void 0 : options.timeout) ?? 10 * 60 * 1e3
            });
...
//# sourceMappingURL=openpipe_openai.js.map
